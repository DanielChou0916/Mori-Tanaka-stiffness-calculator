...
for p=1:crack_num
    ...
    N = 450;
    M = 8;
    [S,P] = Numerical_Eshelby(C,A3D,semi3D,N,M);
...
Mori_Tanaka_stiffness(k).angle=theta2;
Mori_Tanaka_stiffness(k).C_homMT=C_homMT;
Mori_Tanaka_stiffness(k).C_homMT_matrix=C_homMT_matrix;
Mori_Tanaka_stiffness(k).Chom_voigt=Chom_voigt;
Mori_Tanaka_stiffness(k).Chom_voigtPlaneStrain=Chom_voigtPlaneStrain;

end
save('Mori_Tanaka_stiffness.mat','Mori_Tanaka_stiffness');
